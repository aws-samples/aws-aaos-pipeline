...
class TargetStack(Stack):
  def __init__(self, scope: Construct, construct_id: str, pipeline, **kwargs) -> None:
    ...
    target_sg = ec2.SecurityGroup(self, "sec-group",security_group_name="sg_aaos_target", 
      vpc=pipeline.vpc, 
      allow_all_outbound=True)
    
    
    # Allow SSH (TCP port 22) and 8443 traffic
    target_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(22), 'Allow SSH traffic');
    target_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp(8443), 'Allow webUI traffic');

    # Allow TCP and UDP traffic from port 15550 to port 15599
    target_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.tcp_range(15550, 15599), 'Allow webrtc tcp traffic');
    target_sg.add_ingress_rule(ec2.Peer.any_ipv4(), ec2.Port.udp_range(15550, 15599), 'Allow webrtc udp traffic');
    ...
    user_data.add_commands(
      "apt-get update -y",
      "apt-get install -y git devscripts config-package-dev debhelper-compat golang nfs-common binutils u-boot-tools",
      "cd ~",
      ...
      "[Service]",
      "Environment=\"HOME=/stage\"",
      "Type=simple",
      "User=ubuntu",
      "Group=ubuntu",
      "ExecStart=/bin/sh -c \'yes Y | /stage/bin/launch_cvd\'",
      "ExecStop=/stage/bin/stop_cvd",
      "[Install]",
